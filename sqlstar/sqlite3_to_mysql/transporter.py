...
from sqlstar.sqlite3_to_mysql.sqlite_utils import (
    adapt_decimal,
    adapt_timedelta,
    convert_blob,
    convert_decimal,
...
class SQLite3toMySQL:
    ...
    def __init__(self, **kwargs):
        ...
        self._sqlite_tables = (tuple(kwargs.get("sqlite_tables"))
                               if kwargs.get("sqlite_tables") is not None else
                               tuple())

        self._without_foreign_keys = (True if len(self._sqlite_tables) > 0 else
                                      (kwargs.get("without_foreign_keys")
                                       or False))
        ...
        self._logger = self._setup_logger(log_file=kwargs.get("log_file")
                                          or None,
                                          quiet=self._quiet)
    ...
    def _get_mysql_version(self):
        ...
        except (IndexError, connector.Error) as err:
            self._logger.error(
    ...
    def _check_mysql_json_support(version_string):
        ...
        if version_string.lower().endswith("-mariadb"):
            if (mysql_version.major >= 10 and mysql_version.minor >= 2
    ...
    def _check_mysql_fulltext_support(version_string):
        ...
        if version_string.lower().endswith("-mariadb"):
            if (mysql_version.major >= 10 and mysql_version.minor >= 0
    ...
    def _sqlite_table_has_rowid(self, table):
        try:
            self._sqlite_cur.execute(
                """SELECT rowid FROM "{}" LIMIT 1""".format(table))
    ...
    def _translate_type_from_sqlite_to_mysql(self, column_type):
        ...
        if data_type == "UNSIGNED BIG INT":
            return "BIGINT" + self._column_type_length(
    ...
    def _create_table(self, table_name, transfer_rowid=False):
        ...
        for row in self._sqlite_cur.fetchall():
            ...
            sql += " `{name}` {type} {notnull} {auto_increment}, ".format(
                ...
                notnull="NOT NULL"
                if column["notnull"] or column["pk"] else "NULL",
                auto_increment="AUTO_INCREMENT"
                if column["pk"] and self._translate_type_from_sqlite_to_mysql(
                    column["type"]) in {"INT", "BIGINT"} else "",
            )
            if column["pk"] > 0:
                primary_key = {"column": column["name"], "length": ""}
                # In case we have a non-numeric primary key
                column_type = self._translate_type_from_sqlite_to_mysql(
                    column["type"])
        ...
        if len(primary_keys) > 0:
            sql += ", PRIMARY KEY ({columns})".format(columns=", ".join(
        ...
        try:
            ...
            self._logger.error("MySQL failed creating table %s: %s",
                               table_name, err)
            raise
    ...
    def _add_indices(self, table_name):
        ...
        for index in indices:
            ...
            self._sqlite_cur.execute('PRAGMA index_info("{}")'.format(
                index["name"]))
            index_infos = tuple(
                dict(row) for row in self._sqlite_cur.fetchall())

            index_type = "UNIQUE" if int(index["unique"]) == 1 else "INDEX"

            if any(table_columns[index_info["name"]].upper() == "TEXT"
                   for index_info in index_infos):

                if self._use_fulltext and self._mysql_fulltext_support:
                    # Use fulltext if requested and available
                    index_type = "FULLTEXT"
                    index_columns = ",".join("`{}`".format(index_info["name"])
                                             for index_info in index_infos)
                else:
                    # Limit the max TEXT field index length to 255
                    index_columns = ", ".join("`{column}`{length}".format(
                ...
                for index_info in index_infos:
                    ...
                    column_list.append("`{column}`{length}".format(
    ...
    def _add_index(
        ...
        """.format(
            table=table_name,
            index_type=index_type,
            name=index["name"][:64] if index_iteration == 0 else
            "{name}_{index_iteration}".format(name=index["name"][:60],
    ...
    def _add_foreign_keys(self, table_name):
        ...
        for row in self._sqlite_cur.fetchall():
            ...
            """.format(
                ...
                if foreign_key["on_delete"].upper() != "SET DEFAULT" else
                "NO ACTION",
                on_update=foreign_key["on_update"].upper()
                if foreign_key["on_update"].upper() != "SET DEFAULT" else
                "NO ACTION",
            )
    ...
    def transfer(self):
        ...
        if len(self._sqlite_tables) > 0:
            ...
            self._sqlite_cur.execute(
                ...
                """.format(
                    placeholders=("?, " *
                                  len(self._sqlite_tables)).rstrip(" ,")),
                self._sqlite_tables,
            )
        else:
            # transfer all tables
            ...
            for row in self._sqlite_cur.fetchall():
                ...
                self._sqlite_cur.execute(
                    'SELECT COUNT(*) AS total_records FROM "{}"'.format(
                        table["name"]))
                total_records = int(
                    dict(self._sqlite_cur.fetchone())["total_records"])
                ...
                if total_records > 0:
                    ...
                    columns = [
                        column[0] for column in self._sqlite_cur.description
                    ]
                    sql = """
                        INSERT IGNORE
                        INTO `{table}` ({fields})
                    ...
                    """.format(
                        table=table["name"],
                        fields=("`{}`, " *
                                len(columns)).rstrip(" ,").format(*columns),
                        placeholders=("%s, " * len(columns)).rstrip(" ,"),
                    )
                    try:
                        self._transfer_table_data(sql=sql,

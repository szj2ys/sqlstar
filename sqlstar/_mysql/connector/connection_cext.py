...
from .constants import (CharacterSet, FieldFlag, ServerFlag, ShutdownType,
                        ClientFlag)
...
try:
    ...
    from .cursor_cext import (CMySQLCursor, CMySQLCursorRaw,
                              CMySQLCursorBuffered, CMySQLCursorBufferedRaw,
                              CMySQLCursorPrepared, CMySQLCursorDict,
                              CMySQLCursorBufferedDict, CMySQLCursorNamedTuple,
...
class CMySQLConnection(MySQLConnectionAbstract):
    ...
    def fetch_eof_columns(self, prep_stmt=None):
        ...
        for col in fields:
            self._columns.append(
                (col[4], int(col[8]), None, None, None, None,
                 ~int(col[9]) & FieldFlag.NOT_NULL, int(col[9])))
    ...
    def cmd_query(self, query, raw=None, buffered=False, raw_as_string=False):
        ...
        try:
            ...
            raise errors.get_mysql_exception(exc.errno,
                                             msg=exc.msg,
                                             sqlstate=exc.sqlstate)
        except AttributeError:
        ...
        if user_variables or session_variables:
            ...
            if session_variables:
                for key, value in session_variables.items():
                    cur.execute("SET SESSION `{0}` = %s".format(key),
                                (value, ))
            cur.close()

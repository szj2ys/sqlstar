...
class ClientFlag(_Flags):
    ...
    desc = {
        ...
        'SESION_TRACK':
        (1 << 23, 'Capable of handling server state change information'),
    ...
    default = [
        LONG_PASSWD, LONG_FLAG, CONNECT_WITH_DB, PROTOCOL_41, TRANSACTIONS,
        SECURE_CONNECTION, MULTI_STATEMENTS, MULTI_RESULTS, CONNECT_ARGS
    ]
...
class ServerFlag(_Flags):
    ...
    desc = {
        ...
        'SERVER_MORE_RESULTS_EXISTS': (1 << 3, 'Multi query - '
                                       'next query exists'),
        'SERVER_QUERY_NO_GOOD_INDEX_USED': (1 << 4, ''),
        'SERVER_QUERY_NO_INDEX_USED': (1 << 5, ''),
        'SERVER_STATUS_CURSOR_EXISTS':
        (1 << 6, 'Set when server opened a read-only '
         'non-scrollable cursor for a query.'),
        'SERVER_STATUS_LAST_ROW_SENT':
        (1 << 7, 'Set when a read-only cursor is '
         'exhausted'),
        'SERVER_STATUS_DB_DROPPED': (1 << 8, 'A database was dropped'),
        'SERVER_STATUS_NO_BACKSLASH_ESCAPES': (1 << 9, ''),
        'SERVER_STATUS_METADATA_CHANGED':
        (1024, 'Set if after a prepared statement '
         'reprepare we discovered that the '
         'new statement returns a different '
         'number of result set columns.'),
        'SERVER_QUERY_WAS_SLOW': (2048, ''),
        'SERVER_PS_OUT_PARAMS': (4096, 'To mark ResultSet containing output '
                                 'parameter values.'),
        'SERVER_STATUS_IN_TRANS_READONLY': (8192, 'Set if multi-statement '
                                            'transaction is a read-only '
                                            'transaction.'),
        'SERVER_SESSION_STATE_CHANGED':
        (1 << 14, 'Session state has changed on the '
...
class ShutdownType(_Constants):
    ...
    desc = {
        'SHUTDOWN_DEFAULT':
        (SHUTDOWN_DEFAULT, "defaults to SHUTDOWN_WAIT_ALL_BUFFERS"),
        ...
        (SHUTDOWN_WAIT_CRITICAL_BUFFERS, "don't flush InnoDB buffers, "
...
CONN_ATTRS_DN = [
    "_pid", "_platform", "_source_host", "_client_name", "_client_license",
    "_client_version", "_os", "_connector_name", "_connector_license",
    "_connector_version"
]

...
class DatabaseOperations(MySQLDatabaseOperations):
    ...
    def date_trunc_sql(self, lookup_type, field_name):
        ...
        if lookup_type in fields:
            ...
            return "CAST(DATE_FORMAT(%s, '%s') AS DATE)" % (field_name,
                                                            format_str)
        elif lookup_type == 'quarter':
            return "MAKEDATE(YEAR(%s), 1) + INTERVAL QUARTER(%s) QUARTER - INTERVAL 1 QUARTER" % (
                field_name, field_name)
        elif lookup_type == 'week':
            return "DATE_SUB(%s, INTERVAL WEEKDAY(%s) DAY)" % (field_name,
                                                               field_name)
        else:
    ...
    def datetime_trunc_sql(self, lookup_type, field_name, tzname):
        ...
        if lookup_type == 'quarter':
            return ("CAST(DATE_FORMAT(MAKEDATE(YEAR({field_name}), 1) + "
                    "INTERVAL QUARTER({field_name}) QUARTER - " +
                    "INTERVAL 1 QUARTER, '%Y-%m-01 00:00:00') AS DATETIME)"
                    ).format(field_name=field_name)
        if lookup_type == 'week':
            return ("CAST(DATE_FORMAT(DATE_SUB({field_name}, "
                    "INTERVAL WEEKDAY({field_name}) DAY), "
                    "'%Y-%m-%d 00:00:00') AS DATETIME)").format(
                        field_name=field_name)
        try:
            ...
            sql = "CAST(DATE_FORMAT(%s, '%s') AS DATETIME)" % (field_name,
                                                               format_str)
        return sql
    ...
    def time_trunc_sql(self, lookup_type, field_name):
        ...
        if lookup_type in fields:
            format_str = fields[lookup_type]
            return "CAST(DATE_FORMAT(%s, '%s') AS TIME)" % (field_name,
                                                            format_str)
        else:

...
from .cursor import (RE_PY_PARAM, RE_SQL_INSERT_STMT, RE_SQL_ON_DUPLICATE,
                     RE_SQL_COMMENT, RE_SQL_INSERT_VALUES, RE_SQL_SPLIT_STMTS,
                     RE_SQL_FIND_PARAM)
...
class CMySQLCursor(MySQLCursorAbstract):
    ...
    def _fetch_warnings(self):
        ...
        try:
            ...
            raise errors.InterfaceError("Failed getting warnings; {0}".format(
                str(err)))
    ...
    def execute(self, operation, params=(), multi=False):
        ...
        try:
            result = self._cnx.cmd_query(stmt,
                                         raw=self._raw,
    ...
    def _batch_insert(self, operation, seq_params):
        ...
        try:
            ...
            for params in seq_params:
                ...
                if isinstance(prepared, dict):
                    for key, value in prepared.items():
                        tmp = tmp.replace("%({0})s".format(key).encode(),
                                          value)
                elif isinstance(prepared, (list, tuple)):
                    psub = _ParamSubstitutor(prepared)
    ...
    def callproc(self, procname, args=()):
        ...
        try:
            ...
            result = self._cnx.cmd_query(call,
                                         raw=self._raw,
                                         raw_as_string=self._raw_as_string)
            ...
            while self._cnx.result_set_available:
                ...
                if isinstance(self,
                              (CMySQLCursorDict, CMySQLCursorBufferedDict)):
            ...
            if argnames:
                ...
                args = [
                    "{} AS {}".format(name, alias) for name, alias in zip(
...
class CMySQLCursorPrepared(CMySQLCursor):
    ...
    def _handle_eof(self):
        ...
        self._handle_warnings()
        if self._cnx.raise_on_warnings is True and self._warnings:
            raise errors.get_mysql_exception(self._warnings[0][1],
                                             self._warnings[0][2])
    ...
    def _fetch_row(self, raw=False):
        ...
        if self._nextrow == (None, None):
            (row, eof) = self._cnx.get_row(binary=self._binary,
        ...
        if row:
            self._nextrow = self._cnx.get_row(binary=self._binary,
                                              columns=self.description,
                                              raw=raw,
                                              prep_stmt=self._stmt)
            eof = self._nextrow[1]

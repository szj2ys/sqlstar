...
from .constants import (ClientFlag, ServerCmd, ServerFlag, flag_is_set,
                        ShutdownType, NET_BUFFER_LENGTH)
...
from .cursor import (CursorBase, MySQLCursor, MySQLCursorRaw,
                     ...
                     MySQLCursorPrepared, MySQLCursorDict,
                     MySQLCursorBufferedDict, MySQLCursorNamedTuple,
...
class MySQLConnection(MySQLConnectionAbstract):
    ...
    def _add_default_conn_attrs(self):
        ...
        default_conn_attrs = {
            ...
            "_client_version":
            ".".join([str(x) for x in version.VERSION[0:3]]),
            "_os": os_ver
        }
    ...
    def _do_handshake(self):
        ...
        try:
            ...
            raise errors.get_mysql_exception(msg=err.msg,
                                             errno=err.errno,
        ...
        if not handshake['capabilities'] & ClientFlag.SSL:
            ...
            if self._ssl.get('verify_cert'):
                raise errors.InterfaceError(
                    "SSL is required but the server "
                    "doesn't support it",
                    errno=2026)
    ...
    def _do_auth(self,
        ...
        if client_flags & ClientFlag.SSL:
            ...
            self._socket.switch_to_ssl(
                ssl_options.get('ca'), ssl_options.get('cert'),
                ssl_options.get('key'),
                ssl_options.get('verify_cert') or False,
                ssl_options.get('verify_identity') or False, tls_ciphersuites,
            ...
            if new_auth_plugin == "authentication_ldap_sasl_client" \
               ...
               auth_data == b'GSSAPI' and packet[4] != 255:
                ...
                while not complete and tries < 5:
                    _LOGGER.debug("%s Attempt %s %s", "-" * 20, tries + 1,
                                  "-" * 20)
                    _LOGGER.debug("<< server response: %s", packet)
                    _LOGGER.debug("# response code: %s",
                                  packet[:rcode_size + 1])
                    step, complete = auth.auth_continue_krb(
                        packet[rcode_size:])
                ...
                _LOGGER.debug(
                    " last GSSAPI response from server: %s length: %d", packet,
                    len(packet))
                last_step = auth.auth_accept_close_handshake(
                    packet[rcode_size:])
    ...
    def _open_connection(self):
        ...
        try:
            ...
            self._do_handshake()
            self._do_auth(self._user, self._password, self._database,
                          self._client_flags, self._charset_id, self._ssl,
                          self._conn_attrs)
    ...
    def _handle_load_data_infile(self, filename):
        ...
        if os.path.islink(file_name):
            raise errors.OperationalError(
                "Use of symbolic link is not allowed")
    ...
    def cmd_query(self, query, raw=False, buffered=False, raw_as_string=False):
        ...
        try:
            result = self._handle_result(self._send_cmd(
                ServerCmd.QUERY, query))
    ...
    def reset_session(self, user_variables=None, session_variables=None):
        ...
        try:
            ...
            self.cmd_change_user(self._user, self._password, self._database,
                                 self._charset_id)
    ...
    def cursor(self,
        ...
        try:
            ...
            raise ValueError(
                'Cursor not available with given criteria: ' + ', '.join(
                    [args[i]
                     for i in range(5) if cursor_type & (1 << i) != 0]))
    ...
    def cmd_stmt_close(self, statement_id):
        ...
        anything.
        """
        self._send_cmd(ServerCmd.STMT_CLOSE,
    ...
    def cmd_stmt_send_long_data(self, statement_id, param_id, data):
        ...
        try:
            ...
            while buf:
                ...
                self._send_cmd(ServerCmd.STMT_SEND_LONG_DATA,
                               packet=packet,
                               expect_response=False)
                total_sent += len(buf)
                buf = data.read(chunk_size)

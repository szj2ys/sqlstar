...
class MySQLProtocol(object):
    ...
    def _auth_response(self, client_flags, username, password, database,
        ...
        try:
            ...
            raise errors.InterfaceError("Failed authentication: {0}".format(
                str(exc)))
    ...
    def make_auth(self,
        ...
        packet = struct.pack(
            '<IIH{filler}{usrlen}sx'.format(filler='x' * 22,
                                            usrlen=len(username_bytes)),
            client_flags, max_allowed_packet, charset, username_bytes)

        packet += self._auth_response(client_flags, username, password,
                                      database, auth_plugin, auth_data,
                                      ssl_enabled)
    ...
    def parse_handshake(self, packet):
        ...
        (res['server_threadid'], auth_data1, capabilities1, res['charset'],
         ...
         auth_data_length) = struct_unpack('<I8sx2sBH2sBxxxxxxxxxx',
                                           packet[0:31])
    ...
    def parse_ok(self, packet):
        ...
        try:
            ...
            (packet,
             ok_packet['affected_rows']) = utils.read_lc_int(packet[5:])
        ...
        for pair in pairs:
            ...
            try:
                ...
                try:
                    ...
                    raise errors.InterfaceError("{0} ({1}:{2}).".format(
                        errmsg, lbl, val))
        return res
    ...
    def _parse_binary_values(self, fields, packet, charset='utf-8'):
        ...
        for pos, field in enumerate(fields):
            if null_bitmap[int((pos + 2) / 8)] & (1 << (pos + 2) % 8):
                values.append(None)
                continue
            elif field[1] in (FieldType.TINY, FieldType.SHORT, FieldType.INT24,
    ...
    def _prepare_binary_timestamp(self, value):
        ...
        packed = (utils.int2store(value.year) + utils.int1store(value.month) +
    ...
    def _prepare_binary_time(self, value):
        ...
        if isinstance(value, datetime.timedelta):
            ...
            packed += (utils.int4store(abs(value.days)) +
                       utils.int1store(hours) + utils.int1store(mins) +
                       utils.int1store(secs))
            mcs = value.microseconds
        else:
            packed += (utils.int4store(0) + utils.int1store(value.hour) +
                       utils.int1store(value.minute) +
                       utils.int1store(value.second))
        ...
        if parameters and data:
            ...
            for pos, _ in enumerate(parameters):
                ...
                if value is None:
                    ...
                    (packed,
                     field_type) = self._prepare_binary_timestamp(value)
                    values.append(packed)
                ...
                types.append(
                    utils.int1store(field_type) + utils.int1store(flags))

        packet = (utils.int4store(statement_id) + utils.int1store(0) +
                  utils.int4store(iteration_count) +
                  b''.join([struct.pack('B', bit)
                            for bit in null_bitmap]) + utils.int1store(1))

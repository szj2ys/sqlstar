...
class MySQLConnectionAbstract(object):
    ...
    def _validate_tls_ciphersuites(self):
        ...
        translated_names = [
            ":".join(translated_names[0]), ":".join(translated_names[1])
        ]
    ...
    def _validate_tls_versions(self):
        ...
        if isinstance(tls_version, STRING_TYPES):
            if not (tls_version.startswith("[") and tls_version.endswith("]")):
                ...
                for tls_ver in tls_vers:
                    ...
                    if tls_version == "":
                        ...
                        raise AttributeError(
                            TLS_VERSION_ERROR.format(tls_version,
                                                     TLS_VERSIONS))
            ...
            raise AttributeError(
                ...
                "{}. found: '{}'".format(", ".join(TLS_VERSIONS),
                                         tls_versions))
    ...
    def config(self, **kwargs):
        ...
        if set_ssl_flag:
            ...
            if bool('key' in self._ssl) != bool('cert' in self._ssl):
                raise AttributeError("ssl_key and ssl_cert need to be both "
            ...
            if "tls_ciphersuites" in self._ssl and self._ssl[
                    "tls_ciphersuites"] is not None:
                if self._ssl_disabled:
    ...
    def start_transaction(self,
        ...
        if readonly is not None:
            if self._server_version < (5, 6, 5):
                raise ValueError("MySQL server version {0} does not support "

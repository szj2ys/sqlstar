...
class DatabaseIntrospection(BaseDatabaseIntrospection):
    ...
    def get_table_description(self, cursor, table_name):
        ...
        for line in cursor.description:
            ...
            fields.append(
                FieldInfo(name, type_code, display_size,
                          to_int(info.max_len) or line[3],
                          to_int(info.num_prec) or line[4],
                          to_int(info.num_scale) or line[5], line[6],
                          info.column_default, info.extra, info.is_unsigned,
    ...
    def _parse_constraint_columns(self, check_clause, columns):
        ...
        for token in tokens:
            if (token.ttype == sqlparse.tokens.Name and
                    self.connection.ops.quote_name(token.value) == token.value

...
from stringprep import (in_table_a1, in_table_b1, in_table_c11, in_table_c12,
                        in_table_c21_c22, in_table_c3, in_table_c4,
                        in_table_c5, in_table_c6, in_table_c7, in_table_c8,
                        in_table_c9, in_table_c12, in_table_d1, in_table_d2)
import struct
import sys
import unicodedata
    ...
    while pos < buf_len:
        ...
        if first == 251:
            ...
            if first <= 250:
                ...
                byteslst.append(buf[pos + 1 + lsize:length + lsize +
                                    (pos + 1)])
                pos += 1 + lsize + length
...
def _parse_lsb_release_command():
    ...
    with open(os.devnull, "w") as devnull:
        try:
            stdout = subprocess.check_output(("lsb_release", "-a"),
                                             stderr=devnull)
        except OSError:
            return None
...
def linux_distribution():
    ...
    if distro:
        return (distro.get("name", ""), distro.get("version_id", ""),
...
def _get_unicode_read_direction(unicode_str):
    ...
    """
    if unicode_str and unicodedata.bidirectional(
            unicode_str[0]) in ("R", "AL"):
        return "R-to-L"
    return "L-to-R"
...
def _get_unicode_direction_rule(unicode_str):
    ...
    if read_dir == "R-to-L":
        ...
        return {
            "Bidirectional Characters requirement 2 [StringPrep, d2]":
            in_table_d2
        }

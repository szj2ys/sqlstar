...
class MySQLLdapSaslPasswordAuthPlugin(BaseAuthPlugin):
    ...
    def _first_message_krb(self):
        ...
        try:
            ...
            _LOGGER.debug(
                "# Stored credentials found, if password was given it"
                " will be ignored.")
            try:
            ...
            if not self._password:
                _LOGGER.error(" Unable to retrieve stored credentials: %s",
                              err)
                raise errors.InterfaceError(
        ...
        servk = gssapi.Name(service_principal,
    ...
    def auth_accept_close_handshake(self, message):
        ...
        _LOGGER.debug("# wrapped message response: %s, length: %d", wraped[0],
                      len(wraped[0]))
    ...
    def _second_message(self):
        ...
        salted_password = self._hi(passw, b64decode(self.server_salt),
                                   self.iterations)
        ...
        auth_msg = ','.join([
            client_first_no_header, self.servers_first, "c={}".format(
                b64encode("n,a={},".format(self._normalize(
                    self._username)).encode()).decode()),
        ...
        client_header = b64encode("n,a={},".format(
            self._normalize(self._username)).encode()).decode()
        msg = ",".join([
            "c={}".format(client_header), "r={}".format(self.server_nonce),
    ...
    def _validate_second_reponse(self, servers_second):
        ...
        if not servers_second or not isinstance(servers_second, bytearray) or \
           len(servers_second)<=2 or not servers_second.startswith(b"v="):
            raise errors.InterfaceError(
                "The server's proof is not well formated.")

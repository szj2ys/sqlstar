...
class MySQLtoSQLite:
    ...
    def __init__(self, **kwargs):
        ...
        self._mysql_tables = (tuple(kwargs.get("mysql_tables"))
                              if kwargs.get("mysql_tables") is not None else
                              tuple())

        self._without_foreign_keys = (True if len(self._mysql_tables) > 0 else
        ...
        try:
            ...
            except (connector.Error, Exception) as err:
                if hasattr(err,
                           "errno") and err.errno == errorcode.ER_BAD_DB_ERROR:
        ...
        for index in self._mysql_cur_dict.fetchall():
            if int(index["primary"]) == 1:
                primary += "\n\tPRIMARY KEY ({columns})".format(
                    columns=", ".join(
                        '"{}"'.format(column)
                        for column in index["columns"].split(",")))
            else:
                indices += """CREATE {unique} INDEX IF NOT EXISTS "{name}" ON "{table}" ({columns});""".format(
                    ...
                    name="{table}_{name}".format(table=table_name,
                                                 name=index["name"]),
                    table=table_name,
                    columns=", ".join(
                        '"{}"'.format(column)
        ...
        if not self._without_foreign_keys:
            ...
            self._mysql_cur_dict.execute(
                ...
                GROUP BY i.CONSTRAINT_NAME,
                         ...
                         c.DELETE_RULE
                """.format(JOIN="JOIN" if (
                    server_version[0] == 8 and server_version[2] > 19
                ) else "LEFT JOIN"),
    ...
    def _create_table(self, table_name, attempting_reconnect=False):
        try:
            ...
            self._sqlite_cur.executescript(
                self._build_create_table_sql(table_name))
            self._sqlite.commit()
        except connector.Error as err:
            ...
            self._logger.error("SQLite failed creating table %s: %s",
                               table_name, err)
            raise

    def _transfer_table_data(self,
                             table_name,
                             sql,
                             total_records=0,
            ...
            if self._chunk_size is not None and self._chunk_size > 0:
                for chunk in trange(
                    ...
                    self._sqlite_cur.executemany(
                        sql,
                        (tuple(
                            encode_data_for_sqlite(col)
                            if col is not None else None for col in row)
                ...
                self._sqlite_cur.executemany(
                    ...
                    (tuple(
                        ...
                        for col in row) for row in tqdm(
        ...
        try:
            ...
            for table_name in tables:
                ...
                if total_records > 0:
                    ...
                    columns = [
                        column[0] for column in self._mysql_cur.description
                    ]
                    ...
                    self._transfer_table_data(table_name=table_name,
                                              sql=sql,
                                              total_records=total_records)
